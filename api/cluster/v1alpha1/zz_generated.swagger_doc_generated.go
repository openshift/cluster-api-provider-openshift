...
var map_OpenShiftBootstrapConfig = map[string]string{
	"":         "OpenShiftBootstrapConfig is responsible for providing ignition configuration to bootstrp nodes on an OpenShift cluster managed by Cluster API. Compatibility level 4: No compatibility is provided, the API can change at any point for any reason. These capabilities should not be used by applications needing long term support.",
...
var map_OpenShiftControlPlane = map[string]string{
	"":         "OpenShiftControlPlane is responsible for bootstrapping an OpenShift cluster control plane. Compatibility level 4: No compatibility is provided, the API can change at any point for any reason. These capabilities should not be used by applications needing long term support.",
	"metadata": "metadata is the standard object's metadata. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata",
...
var map_OpenShiftControlPlaneSpec = map[string]string{
	...
	"machineTemplate":          "machineTemplate defines the machine template used to create the initial control plane machines. Continued management of the control plane machines will be handled by the control plane machine set. The machine template is therefore immutable and only applicable during the bootstrap process. ",
	"bootstrapMachineTemplate": "bootstrapMachineTemplate defines the machine template used to create the bootstrap machine. The bootstrap machine is used to bootstrap the control plane machines. The machine template is therefore immutable and only applicable during the bootstrap process.",
